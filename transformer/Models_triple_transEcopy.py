...
class Transformer_bert(nn.Module):
    ...
    def forward(self, src_seq, src_pos, src_mask, tgt_seq, tgt_pos, src_triples, src_triples_mask, entitys, entitys_mask, getencoder=False):     #src_turns_label, 
        ...
        print(head)
        print(tail)
        tail_embedding = torch.squeeze(tail) # shape: [?, Nf, 100]
        ...
        if getencoder:
            return feature_enc_output, fact_triples, enfact_weight, tail_embedding

        dec_output, *_ = self.decoder2(tgt_seq, tgt_pos, src_seq, feature_enc_output, src_seq, fact_triples, src_mask, tail_embedding, src_triples_mask)  ###    ##enc_output2    triple state
        #print(dec_output)
    ...
    ####### modified
    def probs(self, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, entity_vocab_map):

        size = list(outputs.size())

        # context_question_weight.shape [?, Ntgt, Nf]
        # context_question_indices.shape [?, Nf]

        # 在这里应该把entity_index 映射为 bert_vocab_index
    ...
    def predict(self, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, ):
