...
class Decoder_bert_stage_2(nn.Module):
    ...
    def forward(self, tgt_seq, tgt_pos, src_seq, enc_output, triples_words, enc_triples, enc_mask, return_attns=False):
        ...
        p_dec_output = self.pointer_layer(dec_output, enc_output,atten_mask = enc_padding)

        if return_attns:
            return p_dec_output, dec_output, dec_slf_attn_list, dec_enc_attn_list, stage_2_attn_list
        return p_dec_output, dec_output
...
class Transformer_bert(nn.Module):
    ...
    def __init__(
        ...
        self.cat_mlp = nn.Sequential(nn.Linear(entity_hidden_size * 3, entity_hidden_size), nn.Tanh())

        # tmp: enhance in the encode
        self.feature_enrich = Feature_enrich(d_model, entity_hidden_size, d_inner, d_k, d_v, dropout)
        self.entity_pointer = PointerDecoder(d_model,d_model,dropout=0.2)

        self.tanh = nn.Tanh()
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=2)
        #fact matching
        self.fact_mlp1 = nn.Linear(d_model+entity_hidden_size, 256)
        self.fact_mlp2 = nn.Linear(256, 1)

        # linear for decoder2
        self.triple_linear = nn.Linear(entity_hidden_size, d_model)
        ...
        # entity copy
        self.entity_word_prj = nn.Linear(d_model, n_tgt_vocab, bias=False)
        nn.init.xavier_normal_(self.entity_word_prj.weight)
        
        # merge two p_vocab
        self.selector = nn.Sequential(nn.Dropout(dropout), nn.Linear(3*d_model, 1), nn.Sigmoid())
    ...
    def decoder(self, tgt_seq, tgt_pos, src_seq, enc_output, fact_triples, src_mask, entitys, entitys_output, entity_mask):
        ...
        dec_output, decoder_output, dec_slf_attn_list, dec_enc_attn_list, stage_2_attn_list = self.decoder2(tgt_seq, tgt_pos, src_seq, enc_output, src_seq, fact_triples, src_mask, return_attns=True)
        cont_ans_outputs, cont_ans_weight, vocab_pointer_switches = dec_output

        enc_padding = entity_mask.data == Constants.PAD
        # 对entity进行copy
        dec_entity_output = self.entity_pointer(decoder_output, entitys_output, atten_mask = enc_padding)
        entity_ans_outputs, entity_ans_weight, entity_vocab_pointer_switches = dec_entity_output
        return cont_ans_outputs, cont_ans_weight, vocab_pointer_switches, entity_ans_outputs, entity_ans_weight, entity_vocab_pointer_switches, decoder_output

    def forward(self, src_seq, src_pos, src_mask, tgt_seq, tgt_pos, src_triples, entitys, entitys_mask, getencoder=False):     #src_turns_label, 

        if tgt_seq is not None: 
            tgt_seq, tgt_pos = tgt_seq[:, :-1], tgt_pos[:, :-1]
        enc_output, *_ = self.encoder(src_seq, attention_mask=src_mask, output_all_encoded_layers=False)  # shape: [?, Nx, 512]

        # 这里可能会有点问题，为什么要用bert？
        entitys_output, *_ = self.encoder(entitys, attention_mask=entitys_mask, output_all_encoded_layers=False)  # shape: [?, Ne, 512]
        #print(entitys_output)

        #########add by wy , knowledge diffusion
        # fact matching
        triples_embedding = self.entity_embed(src_triples)  # shape:[?, Nf, 3, 100]
        # 虽然使用了mean， 但是因为embedding中存在W矩阵，并且参与学习。是不是其实相当于并接起来进行MLP映射？但是缺乏了一个映射层。所以还是加一层MLP比好
        #mean_triples = torch.mean(triples_embedding, dim = -2)  # shape:[?, Nf, 100]
        triples_embedding_shape = triples_embedding.shape
        triples_embedding = triples_embedding.view([triples_embedding_shape[0], triples_embedding_shape[1], triples_embedding_shape[2]*triples_embedding_shape[3]]) # shape:[?, Nf, 300]
        mlp_triples = self.cat_mlp(triples_embedding) # shape:[?, Nx, 100]

        fact_matching, enfact_weight = self.fact_match_cal(enc_output, mlp_triples) # shape:[?, Nx, 100]
        fact_triples = self.triple_linear(fact_matching) # shape: [?, Nx, 512]

        # highway+attn
        feature_enc_output = self.feature_enrich(src_seq, enc_output, fact_matching)

        if getencoder:
            return feature_enc_output, fact_triples, enfact_weight, entitys_output

        dec_output, decoder_output = self.decoder2(tgt_seq, tgt_pos, src_seq, feature_enc_output, src_seq, fact_triples, src_mask)  ###    ##enc_output2    triple state
        #print(dec_output)
        cont_ans_outputs, cont_ans_weight, vocab_pointer_switches = dec_output

        enc_padding = entitys_mask.data == Constants.PAD
        # 对entity进行copy
        dec_entity_output = self.entity_pointer(decoder_output, entitys_output, atten_mask = enc_padding)
        entity_ans_outputs, entity_ans_weight, entity_vocab_pointer_switches = dec_entity_output

        probs, select_prob, entity_select_prob, p_select = self.probs(cont_ans_outputs, vocab_pointer_switches, cont_ans_weight, src_seq, entity_ans_outputs, entity_vocab_pointer_switches, entity_ans_weight, entitys, decoder_output)
        logits = probs.float()
        lprobs = torch.log(logits)
        select_logits = p_select.float()
    ...
    def fact_match_cal(self, enc_output, mean_triples):
        ...
        expand_tri = mean_triples.expand([Nx] + list(mean_triples.shape)).permute(1,0,2,3) # shape: [?, Nx, Nf, 100]
        expand_cat = torch.cat([expand_enc, expand_tri], dim=-1) # shape: [?, Nx, Nf, 612]
    ...
    ####### modified
    def probs(self, outputs, vocab_pointer_switches, cont_ans_weight, src_seq, entity_ans_outputs, entity_vocab_pointer_switches, entity_ans_weight, entitys, decoder_output):
        ...
        scaled_p_vocab.scatter_add_(scaled_p_vocab.dim()-1, src_seq.unsqueeze(1).expand_as(cont_ans_weight),
            (1 - vocab_pointer_switches).expand_as(cont_ans_weight) * cont_ans_weight)

        entity_scores = self.entity_word_prj(entity_ans_outputs.view(-1, entity_ans_outputs.size(-1))).view(size)
        entity_p_vocab = F.softmax(entity_scores, dim=scores.dim()-1)
        entity_scaled_p_vocab = entity_vocab_pointer_switches.expand_as(entity_p_vocab) * entity_p_vocab
        entity_scaled_p_vocab.scatter_add_(entity_scaled_p_vocab.dim()-1, entitys.unsqueeze(1).expand_as(entity_ans_weight),
            (1 - entity_vocab_pointer_switches).expand_as(entity_ans_weight) * entity_ans_weight)
        
        # merge
        p_select = self.selector(torch.cat([decoder_output, outputs, entity_ans_outputs], dim=-1))
        final_p_vocab = p_select * scaled_p_vocab + (1 - p_select) * entity_scaled_p_vocab
        return final_p_vocab, vocab_pointer_switches, entity_vocab_pointer_switches, p_select

    def predict(self, cont_ans_outputs, vocab_pointer_switches, cont_ans_weight, src_seq, entity_ans_outputs, entity_vocab_pointer_switches, entity_ans_weight, entitys, decoder_output):
        probs, select_prob, entity_select_prob, p_select = self.probs(cont_ans_outputs, vocab_pointer_switches, cont_ans_weight, src_seq, entity_ans_outputs, entity_vocab_pointer_switches, entity_ans_weight, entitys, decoder_output)
        lprobs = probs.float()
        lprobs = lprobs.log()
        return lprobs, select_prob, entity_select_prob, p_select
