...
class Transformer_bert(nn.Module):
    ...
    def forward(self, src_seq, src_pos, src_mask, tgt_seq, tgt_pos, src_triples, berttriple_mask, entitys, entitys_mask, getencoder=False):     #src_turns_label, 
        ...
        fact_matching, enfact_weight, hint_logits = self.fact_match_cal(enc_output, mlp_triples, berttriple_mask) # shape:[?, Nx, 100]
        ...
        return lprobs.view(-1,lprobs.size(-1)), select_logits.view(-1, select_logits.size(-1)), hint_logits # hint_logits: [?, Nx, Nf, 1]


    def fact_match_cal(self, enc_output, mean_triples, berttriple_mask):
        ...
        hint_logits = self.fact_mlp2(cat_out) # shape: [?, Nx, Nf, 1]

        # use mask for bert
        # berttriple_mask shape:[?, Nx, Nf]
        berttriple_mask = berttriple_mask.unsqueeze(-1).float()
        expand_berttriple_mask = (1.0 - berttriple_mask) * -10000.0
        masked_hint_logits = hint_logits + expand_berttriple_mask

        weight = self.softmax(masked_hint_logits) 
        masked_weight = weight * (berttriple_mask)

        sum_output = torch.sum(masked_weight * expand_tri, dim=2) # shape: [?, Nx, 100]
        return sum_output, masked_weight, masked_hint_logits
