...
class Transformer_bert(nn.Module):
    ...
    def __init__(
            self,
            n_src_vocab, n_tgt_vocab, n_entity_vocab, len_max_seq, len_max_tgt,bert_model='chinese_L-12_H-768_A-12',
            d_word_vec=512, d_model=512, d_inner=2048,
            n_layers=6, n_head=8, d_k=64, d_v=64, dropout=0.1,
            tgt_emb_prj_weight_sharing=True,
            emb_src_tgt_weight_sharing=True,
        ...
        self.entity_vocab_size = num_entities
    ...
    def forward(self, src_seq, src_pos, src_mask, src_seg_ids, tgt_seq, tgt_pos, src_triples, getencoder=False):     #src_turns_label, 
        ...
        fact_matching, enfact_weight = self.fact_match_cal(enc_output, mean_triples) # shape:[?, Nx, 100]
        ...
        if getencoder:
            return feature_enc_output, fact_triples, enfact_weight
        ...
        probs = self.probs(self.tgt_word_prj,cont_ans_outputs,vocab_pointer_switches, cont_ans_weight,src_seq, self.vocab_size)
        ###############################################
        selector_prob = torch.sigmoid(self.selector(cont_ans_outputs))
        entity_probs = self.probs(self.entity_word_prj,cont_ans_outputs,vocab_pointer_switches, cont_ans_weight,src_seq, self.entity_vocab_size)
        logits = probs.float()
        #lprobs = torch.log(logits)
        entity_logits = entity_probs.float()
        #entity_lprobs = torch.log(entity_logits)
        return logits.view(-1,logits.size(-1)), entity_logits.view(-1,entity_logits.size(-1)), selector_prob.view(-1,selector_prob.size(-1))
    ...
    def probs(self, generator, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, vocab_size, oov_to_limited_idx=None):
        ...
        effective_vocab_size = vocab_size # + len(oov_to_limited_idx) TODO: add the oov
    ...
    def predict(self, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, oov_to_limited_idx=None):
        ...
        word_lprobs = lprobs.log()
        #print(word_lprobs.max(-1))
        ...
        scaled_p_vocab.scatter_add_(scaled_p_vocab.dim()-1, context_question_indices.expand_as(context_question_weight),
            (1 - vocab_pointer_switches).expand_as(context_question_weight) * context_question_weight)
        lprobs = scaled_p_vocab.float()
        entity_lprobs = lprobs.log()
        entity_probs = scaled_p_vocab
        # generate
        selector_prob = torch.sigmoid(self.selector(outputs))
        word_lprobs = (1 - selector_prob) * word_lprobs
        entity_lprobs = selector_prob * entity_lprobs
        lprobs = torch.cat([word_lprobs, entity_lprobs], dim=-1) # shape: [?, Nx, Vw+Ve]
        #lprobs = word_lprobs

        return lprobs, selector_prob, entity_probs
