...
class Decoder_bert_stage_2(nn.Module):
    ...
    def forward(self, tgt_seq, tgt_pos, src_seq, enc_output, triples_words, enc_triples, enc_mask, entitys_output, entitys_mask, return_attns=False):
        ...
        # 只对entity进行copy
        dec_output = self.pointer_layer(dec_output, entitys_output, atten_mask = enc_padding)
...
class Transformer_bert(nn.Module):
    ...
    def __init__(
        ...
        #fact matching
        self.fact_mlp1 = nn.Linear(d_model+entity_hidden_size, 1)
    ...
    def decoder(self, tgt_seq, tgt_pos, src_seq, enc_output, fact_triples, src_mask, entitys, entitys_output, entity_mask):
        ...
        dec_output, dec_slf_attn_list, dec_enc_attn_list, stage_2_attn_list = self.decoder2(tgt_seq, tgt_pos, src_seq, enc_output, src_seq, fact_triples, src_mask, entitys_output, entity_mask, return_attns=True)
    ...
    def forward(self, src_seq, src_pos, src_mask, tgt_seq, tgt_pos, src_triples, entitys, entitys_mask, getencoder=False):     #src_turns_label, 
        ...
        # 这里可能会有点问题，为什么要用bert？因为copynet需要同一模型的输出起作用。
        ...
        fact_matching, enfact_weight = self.fact_match_cal(enc_output, mlp_triples, entitys_mask.float()) # shape:[?, Nx, 100]
        ...
        dec_output, *_ = self.decoder2(tgt_seq, tgt_pos, src_seq, feature_enc_output, src_seq, fact_triples, src_mask, entitys_output, entitys_mask)  ###    ##enc_output2    triple state
        #print(dec_output)
        cont_ans_outputs, cont_ans_weight, vocab_pointer_switches = dec_output
        probs, select_prob = self.probs(cont_ans_outputs, vocab_pointer_switches, cont_ans_weight, entitys)
        logits = probs.float()
        lprobs = torch.log(logits)
        select_logits = select_prob.float()
        #lenfact_weight = torch.log(enfact_weight)
        lenfact_weight = enfact_weight
        return lprobs.view(-1,lprobs.size(-1)), select_logits.view(-1, select_logits.size(-1)), lenfact_weight # hint_logits: [?*Nx, Nf]


    def fact_match_cal(self, enc_output, mean_triples, entitys_mask):
        ...
        hint_logits = self.tanh(self.fact_mlp1(expand_cat)) # shape: [?, Nx, Nf, 256]
        #hint_logits = self.tanh(self.fact_mlp2(cat_out)) # shape: [?, Nx, Nf, 1]
        entitys_mask = (1 - entitys_mask) * -10000
        hint_logits = hint_logits + entitys_mask.unsqueeze(1).unsqueeze(-1)
        weight = self.softmax(hint_logits) 
        lweight = self.log_softmax(hint_logits)

        sum_output = torch.sum(weight * expand_tri, dim=2) # shape: [?, Nx, 100]
        lweight = lweight.squeeze(-1)
        return sum_output, lweight


    ####### modified
    def probs(self, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, oov_to_limited_idx=None):
        ...
        return scaled_p_vocab, vocab_pointer_switches

    def predict(self, outputs, vocab_pointer_switches, context_question_weight, context_question_indices, oov_to_limited_idx=None):
        ...
        lprobs = lprobs.log()
        return lprobs, vocab_pointer_switches
