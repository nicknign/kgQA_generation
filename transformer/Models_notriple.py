...
class Transformer_bert(nn.Module):
    ...
    def __init__(
            self,
            n_src_vocab, n_tgt_vocab, len_max_seq, len_max_tgt,bert_model='chinese_L-12_H-768_A-12',
            d_word_vec=512, d_model=512, d_inner=2048,
            n_layers=6, n_head=8, d_k=64, d_v=64, dropout=0.1,
            tgt_emb_prj_weight_sharing=True,
            emb_src_tgt_weight_sharing=True,
        ...
        #entity diffusion
        self.entity_mlp1 = nn.Linear(d_model+entity_hidden_size+entity_hidden_size, 256)
        self.entity_mlp2 = nn.Linear(256, 1)
    ...
    def forward(self, src_seq, src_pos, src_mask, src_seg_ids, tgt_seq, tgt_pos, src_triples, getencoder=False):     #src_turns_label, 
        ...
        fact_matching = self.fact_match_cal(enc_output, mean_triples) # shape:[?, Nx, 100]
        '''
        # entity diffusion
        '''
        entity_embedding = self.entity_embed(search_entitys) # shape:[?, Ne, 100]
        entity_difussion = self.entity_difussion_cal(enc_output, fact_matching, entity_embedding) # shape:[?, Nx, 100]
        features = torch.cat([fact_matching, entity_difussion], dim=-1)
        '''
        # highway+attn
        ...
        dec_output, *_ = self.decoder(tgt_seq, tgt_pos, src_seq, decoder_input, src_mask)
        ...
        # TODO: how to use entity generate?
    ...
    def fact_match_cal(self, enc_output, mean_triples):
        ...
        weight = self.softmax(self.fact_mlp2(cat_out)) # shape: [?, Nx, Nf, 1]
